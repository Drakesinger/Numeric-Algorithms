...
struct OneNode
{
    ...
    OneNode(double _theta, double _omega, double _l, double _m)
...
{
public:
    TwoNodes(const OneNode& upper, const OneNode& lower,
...
protected:
    ...
    /**
     * Given theta and omega for the upper- and lower-nodes this method computes
     * the numeric derivatives of each one.
     */
    void derivate(const double *yin, double *dydx);
     ...
     */
    virtual void solve(const double *yin, double *yout) = 0;
    ...
    /**
     * Step size to take when numerically solving.
     */
    const double m_dt;
